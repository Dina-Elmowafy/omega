...
const AdminDashboard: React.FC = () => {
  ...
  const handleNewLicensePhotoUpload = () => {
    if (!editingNewLicense) return;
    openImageEditor((croppedImage) => {
      setEditingNewLicense({ ...editingNewLicense, licenseOwnerPhotoUrl: croppedImage });
      setImageEditorOpen(false);
    });
  };
  ...
  const createNewVehicleLicense = (companyName = '') => setEditingNewLicense({ id: '', companyName, personName: '', drivingLicenseNumber: '', plateNumber: '', chassisNumber: '', inspectionDate: '', expiryDate: '', status: 'valid', pdfUrl: '', licenseOwnerPhotoUrl: '' });
        ...
        {activeTab === 'newLicenses' && (
          <div className="space-y-6">
            {!editingNewLicense ? <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
              ...
              <div><label className="block text-sm font-bold mb-2">Person Photo / صورة الشخص</label><button type="button" onClick={handleNewLicensePhotoUpload} disabled={isSavingCert} className="px-4 py-3 bg-slate-900 text-white rounded-lg hover:bg-slate-800 transition disabled:opacity-70">Upload Photo</button><p className="mt-2 text-xs text-gray-500">The photo is saved for the QR license page and is not displayed in the admin table.</p></div>
