...
const NewLicenseView: React.FC = () => {
  ...
  return <div className="min-h-screen bg-gray-50 pt-24 pb-12 px-6 flex justify-center"><div className="bg-white p-8 md:p-12 rounded-2xl shadow-xl w-full max-w-3xl border-t-8 border-omega-yellow">
    ...
    <div className={`mt-6 p-5 rounded-xl border-2 flex items-center justify-between ${active ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}><span className="font-bold">{active ? 'ACTIVE / سارية' : 'NOT ACTIVE / غير سارية'}</span>{active ? <CheckCircle className="text-green-500" /> : <XCircle className="text-red-500" />}</div>
