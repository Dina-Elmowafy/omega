...
const Home: React.FC = () => {
  const { companyInfo, services } = useData();
        ...
        <div className="container mx-auto px-6 relative z-10 text-center">
           ...
           >
             <div className="inline-block px-4 py-1 bg-omega-yellow text-omega-dark font-bold text-sm tracking-widest uppercase mb-6 rounded-full">
               Established {companyInfo.established} • {companyInfo.location}
             </div>
             ...
             <p className="text-xl text-gray-200 mb-10 max-w-2xl mx-auto font-light leading-relaxed">
               {companyInfo.slogan} - Your trusted partner for Inspection, NDT, and Construction Services.
             </p>
        ...
        <div className="container mx-auto px-6">
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
            <div className="max-w-xl">
              ...
              <h2 className="text-4xl md:text-5xl font-display font-bold text-omega-dark dark:text-white">
            ...
            <Link 
              to="/services" 
              className="group hidden md:flex items-center gap-2 text-omega-blue dark:text-omega-yellow font-bold uppercase tracking-wider hover:text-omega-dark transition-colors"
            >
              View All {services.length} Services 
              <span className="bg-omega-blue dark:bg-omega-yellow dark:text-omega-dark text-white rounded-full p-1 transition-transform group-hover:translate-x-1">
                <ArrowRight size={16} />
              </span>
            </Link>
          </div>
        ...
        <div className="container mx-auto px-6">
          <div className="flex flex-col lg:flex-row gap-20 items-center">
            ...
            <div className="lg:w-1/2">
              <span className="text-omega-blue dark:text-omega-yellow font-bold uppercase tracking-widest text-sm mb-2 block">Our Promise</span>
              <h2 className="text-5xl font-display font-bold text-omega-dark dark:text-white mb-10">
                WHY CHOOSE <span className="text-omega-yellow">OMEGA</span>?
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-10 gap-x-8">
                {WHY_CHOOSE_US.map((item, idx) => (
                  <div key={idx} className="flex gap-4 group">
                    <div className="shrink-0 mt-1 p-2 bg-gray-50 dark:bg-slate-800 rounded-lg group-hover:bg-omega-yellow group-hover:text-omega-dark transition-colors text-omega-blue dark:text-omega-yellow">
                      <CheckCircle size={24} />
                    </div>
                    <div>
                      <h4 className="font-bold text-gray-900 dark:text-white text-lg mb-2 font-display uppercase tracking-wide">{item.title}</h4>
            ...
            <div className="lg:w-1/2 relative">
               ...
               >
                 <img 
                   src="/imge/why-chose-omega.png" 
                   alt="Rope Access Worker" 
                   className="rounded-2xl shadow-2xl"
                 />
                 
                 {/* Floating Stat Card */}
                 <div className="absolute -bottom-10 -left-10 bg-white dark:bg-slate-800 p-8 rounded-xl shadow-2xl border-l-8 border-omega-yellow hidden md:block">
                   <div className="flex items-center gap-4 mb-2">
                     <Shield size={32} className="text-omega-blue dark:text-omega-yellow" />
                     <span className="text-4xl font-bold font-display text-omega-dark dark:text-white">100%</span>
                   </div>
                   <p className="text-xs font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400">Safety Compliance Record</p>
                 </div>
               </motion.div>
            </div>
